...
try {
    $stmt = $pdo->prepare("UPDATE entradas SET valor = :valor, descricao = :descricao, updated_at = CURRENT_TIMESTAMP WHERE id = :id AND user_id = :user_id");
    $stmt->execute([
        ':valor'     => $valor,
        ':descricao' => $descricao,
        ':id'        => $id,
    ...
    if ($stmt->rowCount() > 0) {
        ...
        echo json_encode(['success' => false, 'message' => 'Entrada não encontrada ou nenhum dado alterado.']);
    }
